#!/usr/bin/python3
"""
    This file is only for labeling the BDD100k database. It contains all the semantic labels
    notations, as {name : (colorR, colorG, colorB)}
    Research for multi-class: https://www.fer.unizg.hr/_download/repository/IMVIP_2017_HrkacBrkicKalafatic.pdf

    This file was taken from: https://github.com/bdd100k/bdd100k/blob/master/bdd100k/label/label.py
    Currently, the algorithm supports only 7 categories
"""
...
labels = [
    ...
    Label("sky", 30, 10, "sky", 5, False, False, (70, 130, 180)),
    Label("person", 31, 11, "human", 6, True, False, (220, 20, 60)),
    Label("car", 33, 18, "vehicle", 7, True, False, (119, 11, 32)),
]
"""
    This function is designed only for BDD100k. It should be processed with this databased.
"""
def GetColorCategory() -> dict:
    """
        This function will take a color label, and break it into more channels, each channel for a class,
        according to the input dictionary.

        This is currently designed only for bdd100k!
    """
    colorCategory = {}
    for val in labels:
        if val.color not in colorCategory.keys():
            colorCategory[val.color] = val.categoryId
    return colorCategory

def GetColorIdentity() -> dict:
    colorCategory = {}
    for val in labels:
        if val.categoryId not in colorCategory.keys():
            colorCategory[val.categoryId] = val.color
    return colorCategory

def ReconstructImageByCategory(featureMap):
    print(featureMap.shape)
    width, height = featureMap.shape[0], featureMap.shape[1]
    #featureMap = featureMap[0]

    categoryMap = GetColorIdentity()
    # channels, height, width
    newImage = numpy.empty((512, 512, 3), dtype=numpy.uint8)
    print(featureMap)
    for y in range(height):
        for x in range(width):
            v = featureMap[x][y].item()
            pixel = categoryMap[v]
            #print(pixel)
            newImage[x][y] = pixel
    return newImage